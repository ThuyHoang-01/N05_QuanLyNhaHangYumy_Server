...
const order= {
    ...
    newOrderRequest: expressAsyncHandler(async (req, res)=> {
        try {
            const [rows]= await connection.execute("INSERT INTO order_request(order_request_id, user_name, phone, email , deposit, time_created) VALUES(?, ?, ?, ?, ?, ?)", [v4(), req.body.user_name, req.body.phone, req.body.email, req.body.deposit=== true ? 1 : 0, req.body.time_created])
